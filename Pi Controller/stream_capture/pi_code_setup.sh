...
for i in {1..8}; do
  cp livestream.sh "./PiCode/${i}livestream.sh"

  new_port=$((start_port + i))

  sed -i '' "s/PORT_PLACEHOLDER/$new_port/" "./PiCode/${i}livestream.sh"
  sed -i '' "s/IP_ADDRESS_PLACEHOLDER/$live_ip/" "./PiCode/${i}livestream.sh"
  sed -i '' "s/SLEEP_TIME_PLACEHOLDER/$sleep_time/" "./PiCode/${i}livestream.sh"
done
